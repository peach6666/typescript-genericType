...
export let data= new DataCollection<Person,City>(people)
console.log(`Names:${data.getNames().join(",")}`)
//每次建立新的物件實字，都可以用不同型帶替代泛型參數 
export let data2=new DataCollection<Product,City>(products)
//不給泛型引數，TS也會隱性推論
...
//子類別繼承泛型
class ForExtend<T extends {name:string,city:string}>{
    public project:T[]=[]
    constructor(element:T[]){
        this.project.push(...element)
    }
}
class UseForExtend<T extends {name:string,city:string}> extends ForExtend<T>{
    constructor(element:T[]){
        super(element)
    }
    findPerson(personName:string){
...
let peopleData=new UseForExtend<Person>(people)
